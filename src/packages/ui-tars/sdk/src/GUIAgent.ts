...
import { IMAGE_PLACEHOLDER, MAX_LOOP_COUNT } from '../../shared/src/types';
...
> {
    ...
    try {
      ...
      while (true) {
        ...
        for (const parsedPrediction of parsedPredictions) {
          ...
          // Stop processing if we're in the process of aborting to prevent duplicate commands
          if (this.isAborting) {
            logger.info('[GUIAgent] Skipping action execution during abort');
            break;
          }
          ...
          if (!signal?.aborted && !this.isStopped) {
            ...
            // Track pending operations to ensure proper cleanup during abort
            ...
            try {
              ...
              // Always decrement pending operations count
              this.pendingOperations--;
              logger.info(`[GUIAgent] Completed operation, ${this.pendingOperations} remaining`);
            }
          }
      ...
      if (
        ...
        if (this.pendingOperations > 0) {
          logger.info(`[GUIAgent] Waiting for ${this.pendingOperations} pending operations to complete before finalizing abort`);

          // Wait up to 1 second for pending operations to complete
          const startWait = Date.now();
          while (this.pendingOperations > 0 && Date.now() - startWait < 1000) {
            await new Promise(resolve => setTimeout(resolve, 50));
          }

          if (this.pendingOperations > 0) {
            logger.warn(`[GUIAgent] Aborted with ${this.pendingOperations} operations still pending`);
          } else {
            logger.info('[GUIAgent] All pending operations completed before abort finalized');
          }
        }
  ...
  public stop() {
    ...
    // Wait for any pending operations to complete
    if (this.pendingOperations > 0) {
      this.logger.info(`[GUIAgent] Waiting for ${this.pendingOperations} pending operations to complete before stopping`);

      // Return a promise that resolves when all pending operations are complete
      return new Promise<void>((resolve) => {
        const checkInterval = setInterval(() => {
          if (this.pendingOperations <= 0) {
            clearInterval(checkInterval);
            this.logger.info('[GUIAgent] All pending operations completed, stop confirmed');
            this.isAborting = false;
            resolve();
          }
        }, 100);
        ...
        setTimeout(() => {
          clearInterval(checkInterval);
          this.logger.warn('[GUIAgent] Stop timeout reached with pending operations');
          this.isAborting = false;
          resolve();
        }, 1000);
      });
