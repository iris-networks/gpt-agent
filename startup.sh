...
# Restrict headless user access to /app directory
if id "headless" &>/dev/null; then
    # Ensure the headless user exists before trying to restrict access
    chmod 700 /app
    chown -R root:root /app
fi

# Start the Node.js application as root
if [ -f /app/dist/main/main.js ]; then
    echo "Starting Node.js application as root"
    su -c "node /app/dist/main/main.js" root &
    NODE_PID=$!
